...
| 4A.3 | Story | Remote (CPU): build full residual-target dataset on OTM (per-date RBF at query coords → residuals) + finiteness/MAE-sanity audit — built 10.53M rows, 0 non-finite, val/test mean\|resid\| == 3X.6 RBF MAE; parquet on persistent /workspace | `done` | `docs/tasks/specs/4A.3_remote_build_residual_dataset.md` | 2026-06-20 |
