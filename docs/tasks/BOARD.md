...
| 4B | Epic | **Phase 4 follow-up — sparsity-sweep diagnostic** (accuracy-survival gate per ADR 0011): RBF vs RBF-prior hybrid over increasing sparsity / thin wings / missing maturities. **REOPENED 2026-06-22 — 4B.5 gate `ambiguous`; operator chose the FULL fair retrain (4B.7) to resolve the eval-time OOD ambiguity (16 cells, GPU). The 4B.6 close / ADR 0011 retired-verdict are provisional pending 4B.7.** | `in_progress` | `docs/roadmaps/phase4b_sparsity_sweep.md` | 2026-06-22 |
...
| 4B.6 | Story | Local: Phase 4B closing addendum + ADR 0011 Outcome + journal; flip epic 4B done — **REOPENED 2026-06-22: provisional close reversed; will re-close after 4B.7 fair retrain resolves the gate** | `in_progress` | `docs/tasks/specs/4B.6_local_phase4b_close.md` | 2026-06-22 |
| 4B.7 | Story | Remote (GPU): per-regime fair-retrain escalation (resolve 4B.5 `ambiguous`) — **FULL retrain: all 4 regimes × 4 non-dense rungs = 16 cells; retrain hybrid on sparse context (mirror 4A.4), rescore, recompute edge + CIs, resolve verdict** | `in_progress` | `docs/tasks/specs/4B.7_remote_conditional_retrain_escalation.md` | 2026-06-22 |
