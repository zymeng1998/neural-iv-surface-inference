...
| 4B | Epic | **Phase 4 follow-up — sparsity-sweep diagnostic** (accuracy-survival gate per ADR 0011): RBF vs RBF-prior hybrid over increasing sparsity / thin wings / missing maturities. **CLOSED 2026-06-21 — verdict `ambiguous`, accuracy retired on this substrate; 4B.7 declined; pivot to Phase 5 reliability-first (coverage collapses 0.96→0.35 under sparsity). RBF-prior hybrid stays adopted (ADR 0010 unchanged).** | `done` | `docs/roadmaps/phase4b_sparsity_sweep.md` | 2026-06-21 |
...
| 4B.6 | Story | Local: Phase 4B closing addendum + ADR 0011 Outcome + journal; flip epic 4B done — **ADR 0011 Outcome filled (ambiguous → accuracy retired); roadmap close §7; README pivot to Phase 5** | `done` | `docs/tasks/specs/4B.6_local_phase4b_close.md` | 2026-06-21 |
| 4B.7 | Story | Remote (GPU): **CONDITIONAL** per-regime retrain escalation — only if 4B.5 returns `ambiguous`; else `cancelled` — **DECLINED by operator (economically marginal fair-retrain upside); not run** | `cancelled` | `docs/tasks/specs/4B.7_remote_conditional_retrain_escalation.md` | 2026-06-21 |
