...
| 4A | Epic | **Phase 4 — RBF-prior hybrid / residual neural model** (per ADR 0004 / 0009 / 0010): σ̂ = RBF + f_θ(residual) + calibrated reliability layer. **CLOSED positive: bar MET — hybrid 0.006006 vs RBF 0.006132, 95% CI [−0.000144,−0.000106] significant; reliability preserved. Recommend adopting the hybrid (ADR 0010 Implemented; memo `docs/phase4_result_memo.md`).** | `done` | `docs/roadmaps/phase4_hybrid_residual.md` | 2026-06-20 |
...
| 4B | Epic | **Phase 4 follow-up — sparsity-sweep diagnostic** (accuracy-survival gate per ADR 0011): RBF vs RBF-prior hybrid over increasing sparsity / thin wings / missing maturities. **CLOSED 2026-06-22 — gate `ambiguous` → `true` (accuracy SURVIVES) after full fair retrain (4B.7). Trained fairly, hybrid edge grows under structured sparsity: thin_wings +16.6%, missing_maturities +37.1%, combined +16.0% at sparsest (all sig); eval-time wing collapse was an OOD artifact. Phase 5 reliability-first proceeds in parallel.** | `done` | `docs/roadmaps/phase4b_sparsity_sweep.md` | 2026-06-22 |
...
| 4B.2 | Story | Local: sparsity-sweep harness (fixed-query / shrinking-context) + 4 regimes + unit/smoke tests (no full build, no checkpoint) — 26 harness tests + full suite 465 green; smoke manifest emitted | `done` | `docs/tasks/specs/4B.2_local_sparsity_sweep_harness.md` | 2026-06-21 |
| 4B.3 | Story | Remote (CPU): materialize swept eval inputs on full OTM — per-rung RBF at fixed query coords + finiteness/MAE-sanity audit — **694 test dates; 0 non-finite; RBF MAE monotone; dense rung == 4A floor 0.006132 (err 5e-18); 492 MB swept parquet on Pod for 4B.4** | `done` | `docs/tasks/specs/4B.3_remote_build_swept_eval_inputs.md` | 2026-06-21 |
| 4B.4 | Story | Remote: run the 4A hybrid checkpoint forward across all rungs → σ̂ predictions (eval-time; only checkpoint-dependent story) — **694 test dates; 0 non-finite; hybrid MAE monotone; dense rung == 4A hybrid floor 0.006006 (err 3e-9); 1.38 GB pred parquet pulled local for 4B.5** | `done` | `docs/tasks/specs/4B.4_remote_hybrid_forward_sweep.md` | 2026-06-21 |
| 4B.5 | Story | Local: edge-vs-sparsity trajectory + date-clustered bootstrap CIs (mirror 4A.7) + ADR 0011 gate adjudication — **VERDICT `ambiguous` → triggers 4B.7; rel edge grows only for fewer_quotes (to 4.3%, sub-5%), collapses for wing regimes (to 0.3-0.5%); coverage collapses 0.96→0.35; dense ties 4A exactly** | `done` | `docs/tasks/specs/4B.5_local_trajectory_and_gate.md` | 2026-06-21 |
| 4B.6 | Story | Local: Phase 4B closing addendum + ADR 0011 Outcome + journal; flip epic 4B done — **re-closed POSITIVE 2026-06-22: gate resolved `true` (accuracy survives); ADR 0011 Outcome rewritten; roadmap §7 + README updated** | `done` | `docs/tasks/specs/4B.6_local_phase4b_close.md` | 2026-06-22 |
| 4B.7 | Story | Remote (GPU): per-regime fair-retrain escalation (resolve 4B.5 `ambiguous`) — **DONE: full 16-cell fair retrain (RTX 4000 Ada, 6.3h GPU); gate `ambiguous` → `true`; eval-time wing collapse was OOD; fair edge +16-37% at sparsest** | `done` | `docs/tasks/specs/4B.7_remote_conditional_retrain_escalation.md` | 2026-06-22 |
