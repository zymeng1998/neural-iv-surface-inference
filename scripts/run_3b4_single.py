#!/usr/bin/env python3
"""3B.4 single-config runner: train + score val/test + emit CSVs + manifest.

Clone of ``scripts/run_2d7_single.py``; extends the local
``_build_model_from_cfg`` helper to forward ``decoder_kind``, ``anp``, and
``coord_encoding`` so the ANP cross-attention path activates end-to-end
when reloading the best checkpoint for scoring.

Usage:
    python scripts/run_3b4_single.py --config configs/conditional_3B4_anp_gaussian.yaml
    python scripts/run_3b4_single.py --config configs/conditional_3B4_anp_quantile.yaml
    python scripts/run_3b4_single.py --config configs/conditional_3B4_anp_point_control.yaml
...
def _build_model_from_cfg(cfg: dict, head_cfg: dict) -> ConditionalSurfaceModel:
    """Reconstruct a ConditionalSurfaceModel from a persisted training config.

    Forwards decoder_kind / anp / coord_encoding so the ANP path round-trips
    through checkpoint reload — the 3B.4 entry-point that the 2D.7 helper
    silently dropped.
    """
...
def main() -> int:
    ...
    results_dir = Path(paths.get("results_dir", "artifacts/runs/3B4/_unknown"))
    ...
    if not bench_file.exists():
        print(f"[3B.4] benchmark not found: {bench_file}", file=sys.stderr)
        return 2

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(
        f"[3B.4] head_kind={head_kind}  decoder_kind={cond_cfg.get('decoder_kind')}  "
    ...
    print(
        f"[3B.4] rows: train={len(train_df):,}  val={len(val_df):,}  test={len(test_df):,}"
    )
    ...
    print(f"[3B.4] training done in {train_elapsed:.1f}s  epochs={len(history)}")
    ...
    print(
        f"[3B.4] head={head_kind}  test_MAE_mu={test_mae:.6f}  "
