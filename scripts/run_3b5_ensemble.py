#!/usr/bin/env python3
"""3B.5 K-seed ANP ensemble: train K members + score ensemble + emit CSVs.

Clone of ``scripts/run_2d8_ensemble.py``; identical logic, but
- tags the manifest with ``story = "3B.5"``;
...
Usage:
    python scripts/run_3b5_ensemble.py --config configs/conditional_3B5_anp_ensemble.yaml
...
def main() -> int:
    ...
    results_dir = Path(paths.get("results_dir", "artifacts/runs/3B5"))
    ...
    if not bench_file.exists():
        print(f"[3B.5] benchmark not found: {bench_file}", file=sys.stderr)
        return 2
    ...
    print(
        f"[3B.5] device={device}  size={size}  seeds={seeds}  "
    ...
    print(
        f"[3B.5] rows: train={len(train_df):,}  val={len(val_df):,}  test={len(test_df):,}"
    )
    ...
    for seed in seeds:
        ...
        print(
            f"[3B.5] seed={seed} done  best_val={best_val:.6f}  "
    ...
    print(f"[3B.5] wrote {members_json}")
    ...
    # ── Score ensemble ───────────────────────────────────────────────
    print("[3B.5] loading members for scoring …")
    ...
    print(
        f"[3B.5] ensemble test_MAE={test_mae:.6f}  val_MAE={val_mae:.6f}  "
