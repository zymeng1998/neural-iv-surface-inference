#!/usr/bin/env python3
"""3X.8 single-head runner: DeepSets/mean-pool conditional on OTM.

Byte-for-byte the proven 2D.7 runner logic (scripts/run_2d7_single.py),
re-stamped story="3X.8" so the manifest attributes the run correctly. The
DeepSets mean-pool decoder is the model default (decoder_kind unset), exactly
as the original 2D.7 runs — this is a faithful restatement on the OTM
benchmark, not a new architecture.

Usage:
    python scripts/run_3x8_deepsets.py --config configs/conditional_3X8_deepsets_gaussian_otm.yaml
    python scripts/run_3x8_deepsets.py --config configs/conditional_3X8_deepsets_quantile_otm.yaml
    python scripts/run_3x8_deepsets.py --config configs/conditional_3X8_deepsets_point_control_otm.yaml
...
def _score_split(
    ...
    """Per-row predictions on a benchmark frame, iterating by date."""
    model.eval()
    n = len(df)
...
def main() -> int:
    ...
    results_dir = Path(paths.get("results_dir", "artifacts/runs/3X8/_unknown"))
    ...
    if not bench_file.exists():
        print(f"[3X.8] benchmark not found: {bench_file}", file=sys.stderr)
        return 2

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"[3X.8] head_kind={head_kind}  device={device}  benchmark={bench_file}")
    ...
    print(
        f"[3X.8] rows: train={len(train_df):,}  val={len(val_df):,}  test={len(test_df):,}"
    )
    ...
    print(f"[3X.8] training done in {train_elapsed:.1f}s  epochs={len(history)}")
    ...
    print(
        f"[3X.8] head={head_kind}  test_MAE_mu={test_mae:.6f}  "
